# Linked Lists

This folder contains C programs based on **linked list data structures**, including both **single linked lists** and **doubly linked lists**. It demonstrates various operations such as insertion, traversal, searching, sorting, and reversing.

These programs help in understanding dynamic memory allocation and pointer-based data structures.

---

## 📚 Topics Covered

- Single Linked List (SLL)
- Doubly Linked List (DLL)
- Dynamic memory allocation
- Insertion operations
- Traversal
- Searching
- Sorting
- Reversing linked lists

---

## 📂 Program List

| S. No. | Questions | File Name |
| --- | --- | --- |
| 1. | Write a C program to create a single linked list and perform insertion at beginning, insertion at end, and traversal. | [sll_basic_operations.c]() |
| 2. | Write a C program to perform search, sort, reverse, and traversal operations on a single linked list using menu-driven approach. | [sll_advanced_operations.c]() |
| 3. | Write a C program to create and display a doubly linked list using user-defined functions. | [dll_creation_traversal.c]() |
| 4. | Write a C program to reverse a doubly linked list. | [dll_reverse.c]() |

---

## 🎯 Purpose

This collection helps in:

- Understanding pointer-based data structures
- Learning dynamic memory management using `malloc`
- Performing efficient data manipulation
- Building strong fundamentals for advanced data structures

---
