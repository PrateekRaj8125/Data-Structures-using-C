# Queues and Circular Queues

This folder contains C programs based on the **queue data structure**, including implementations using arrays, linked lists, and circular queues.

These programs demonstrate fundamental queue operations and variations, helping in understanding FIFO-based data handling.

---

## 📚 Topics Covered

- Queue implementation using arrays
- Queue implementation using linked lists
- Circular queue using linked list
- Insert (Enqueue) operation
- Delete (Dequeue) operation
- Display queue elements
- Queue overflow and underflow
- Checking if queue is empty

---

## 📂 Program List

| S. No. | Questions | File Name |
| --- | --- | --- |
| 1. | Write a menu-driven C program to implement queue operations (insert, delete, display, check empty) using array. | [queue_array_operations.c]() |
| 2. | Write a menu-driven C program to implement queue operations using linked list. | [queue_linkedlist_operations.c]() |
| 3. | Write a C program to implement a circular queue using linked list. | [circular_queue_linkedlist.c]() |
| 4. | Write a C program to implement and demonstrate circular queue operations using linked list. | [circular_queue_operations.c]() |

---

## 🎯 Purpose

This collection helps in:

- Understanding FIFO (First In First Out) data structures
- Learning static vs dynamic queue implementations
- Handling circular data structures efficiently
- Building foundation for scheduling and buffering systems

---
